...
OUTPUT_FILE = Path("data/processed/hotspot_features_2025_v2.csv")
...
]  # fire_type deliberately NOT read - see the v2 note below
DTYPES = {
...
for chunk_number, chunk in enumerate(reader, start=1):
    ...
    # NO TYPE FILTER (v2).
    #
    # v1 kept only fire_type 0 and 2 here, which meant every feature below was
    # computed on a NASA-classified subset: `total_detections` really meant
    # "detections NASA typed as 0 or 2". That made the evidence conditional on
    # the very classification we are trying to learn independently, and it
    # discarded all 151,585 offshore detections - where gas flares live.
    #
    # Every physically detected thermal anomaly now contributes.
    if chunk.empty:
        continue
...
# ---- No label here (v2) ----------------------------------------------------
# v1 wrote `type2_fraction` and a `label` derived from it. Both are gone: the
# stage-1 target now comes from field-verified IHS records, joined separately,
# so nothing in this file carries NASA's classification.
...
print("  (no labels written - the target is joined from verified IHS records)")
